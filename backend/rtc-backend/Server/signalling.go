...
func JoinRoomRequestHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // Generate and send ICE candidates from the server
  go generateServerICECandidates(peerConnection, ws)
...
// Generates ICE candidates on the signaling server itself
func generateServerICECandidates(peerConnection *webrtc.PeerConnection, ws *websocket.Conn) {
  webrtcMutex.Lock()
  ...
  peerConnection.OnICECandidate(func(candidate *webrtc.ICECandidate) {
    ...
    // Send ICE candidate to the client via WebSocket
    candidateMsg := map[string]interface{}{
      "type":      "iceCandidate",
      "candidate": candidate.ToJSON(),
    }

    err := ws.WriteJSON(candidateMsg)
    if err != nil {
