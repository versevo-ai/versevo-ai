...
class userViews(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if userform_data.is_valid():
            ...
            if raw_user_data.is_valid():
                ...
                user_obj = NewUser.objects.filter(
                    username=fetched_data.get("username")
                ).all()
                login(request, user_obj) # type: ignore
                return obj
            else:
                raise raw_user_data.messagestack # type: ignore
    ...
    def patch(self, request):
        ...
        if request.user is not None:
            updateuser_data = updateUserForm(
                user=request.user, data=request.POST or None # type: ignore
            )
            if updateuser_data.is_valid():
                ...
                if raw_user_data.is_valid():
                    if (
                        ...
                        != NewUser.objects.get(username=request.user.username).password
                    ):
                        form = PasswordChangeForm(
                            user=request.user, data=raw_user_data.password
                        )
                        if form.is_valid():
                            form.save()
                            update_session_auth_hash(request, form.user)
                            obj = raw_user_data.update_user_api() # type: ignore
                            return obj
                        else:
